...
@dataclass
class MDConfig:
    backend: str = "openmm"              # openmm | schrodinger
    schrodinger: str = "/opt/schrodinger2025-2"
    cpu_jobs: int = 3
    gpu_index: int = 0
...
class HemobindConfig:
    ...
    def validate(self):
        ...
        if self.md.backend == "schrodinger":
            schrodinger = Path(self.md.schrodinger)
            if not schrodinger.exists():
                raise FileNotFoundError(f"Schrodinger not found: {self.md.schrodinger}")
...
def load_config(path: str | Path) -> HemobindConfig:
    ...
    if "md" in raw:
        m = raw["md"]
        cfg.md = MDConfig(
            backend=m.get("backend", "openmm"),
            schrodinger=m.get("schrodinger", "/opt/schrodinger2025-2"),
