"""
pipeline.py — Stage orchestrator with checkpointing.

Each stage writes a `{stage}.done` file on success.
On `--from s3`, stages s1..s2 are skipped if `.done` exists.
"""
...
class Pipeline:
    ...
    def _write_run_meta(self):
        meta = {
            ...
            "backend": self.config.md.backend
        }
    ...
    def run(self, from_stage: str | None = None, to_stage: str | None = None):
        ...
        self.log.info(f"Backend: {self.config.md.backend}")
        ...
        if context_file.exists():
            try:
                ...
                # Convert strings back to Path where possible
                for k, v in raw_context.items():
                    if isinstance(v, str) and (v.startswith("/") or v.startswith("./")):
                        context[k] = Path(v)
                    elif isinstance(v, dict):
                        # Handle nested dicts (like lig_pdbqts)
                        context[k] = {nk: Path(nv) if isinstance(nv, str) and (nv.startswith("/") or nv.startswith("./")) else nv for nk, nv in v.items()}
                    else:
                        context[k] = v
                self.log.info("Loaded context from context.json")
    ...
    def _get_stage_fn(self, stage: str):
        if stage == "s1_prepare":
            ...
            if self.config.md.backend == "openmm":
                from hemobind.stages.openmm.s5_prep import run as fn
            else:
                from hemobind.stages.schrodinger.s5_prepwiz import run as fn
        elif stage == "s6":
            if self.config.md.backend == "openmm":
                from hemobind.stages.openmm.s6_build import run as fn
            else:
                from hemobind.stages.schrodinger.s6_build import run as fn
        elif stage == "s7":
            if self.config.md.backend == "openmm":
                from hemobind.stages.openmm.s7_md import run as fn
            else:
                from hemobind.stages.schrodinger.s7_md import run as fn
        else:
